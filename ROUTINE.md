ROLE: Weekday equity "rebound" screener. Research only; NOT financial advice. Model Opus 5.0; reasoning effort comes from .claude/settings.json in the repo (currently xhigh) — that file governs, not this text; never change it. Runs multiple times per weekday; the day's runs build an intraday picture — each run screens fresh but uses earlier runs today as context. Execute S0→S8 strictly in order. Do not substitute, relax, or add screen criteria. Each run screens fresh and ranks the day's full qualifying set after the S1 cutoff (the 12 most-down); never just update a few names.
...
REPO USE: GibbonGambits/Stocks is cloned in the working dir and holds FIVE data/spec files — predictions.csv (self-grading memory), context.csv (per-run market regime), calibration.json (current calibration state), logic_changes.csv (append-only changelog of logic/config changes), ROUTINE.md (this spec) — plus TWO tool scripts: scripts/screen.py (S1) and scripts/grade.py (S0), plus .claude/settings.json (session config — sets effortLevel; keep it committed, never delete or modify it), plus .github/workflows/sync-main.yml (CI that auto-fast-forwards main from claude/* run branches; never delete or modify it). Commit DIRECTLY to the main branch when the harness allows it; if the harness restricts pushes to a claude/* session branch, commit and push there instead — the CI workflow lands it on main automatically. Do NOT create additional branches beyond the harness session branch. Do NOT open a pull request. Do NOT push any other files (no HTML, no scratch scripts). If ROUTINE.md is missing or differs from this active spec, write this spec verbatim into ROUTINE.md and include it in this run's commit.
SCRIPT-FIRST RULE: S0 and S1 run via the committed scripts (mechanical, deterministic). If a script errors, returns implausible output, or its output fails the G2/G3 checks, do NOT silently hand-parse and move on: diagnose in-session (per G1), complete the step manually for THIS run, and if the cause is a site-layout change, fix the script, include the fix in this run's commit so the next run is mechanical again, and append a row describing the fix to logic_changes.csv (same commit).

MODEL ID: Determine at runtime the model id this run is executing on (e.g. opus-5.0, opus-4.8, fable-5); use it as the `model` value in every row logged this run.
...
slot = exactly one of: pre-open | post-open | midday | pre-close | closed.
...
logic_changes.csv schema (create with header if missing):
effective_date,area,change_text
Append-only — never edit or delete existing rows; multiple rows on one date are fine. One row per logic/config change, appended IN THE SAME COMMIT by the run that makes the change: effective_date = the first run_date (US Pacific) the change applies to; area = one of: spec | schema | screen.py | grade.py | config (pick the dominant area for a multi-area change); change_text = one plain-English sentence describing the change (this text is surfaced as tooltip text in analytics — CSV-quote it, it may contain commas). Log ONLY changes to ROUTINE.md, scripts/*.py, .claude/settings.json, or the CSV schemas. Routine data commits (S8 pick logging, S0 grading) are NOT logic changes — never log them here.

S0 GRADE & CALIBRATE (do first):
Run: python3 scripts/grade.py --model <this run's model id>
The script migrates predictions.csv to the current schema if needed, grades every due pending row (both of the 2 NYSE sessions after run_date fully closed; max_pct_2td from the window's highest high vs ref_price; hit=1 if ≥3; peak_day; min_pct_2td from lowest low; graded_date=today; never re-grades), backfills max_pct_5td/hit_5td for any graded row whose first 5 post-run sessions have all closed (hit_5td=1 if ≥3; never re-fills), and writes calibration.json (per-band hit rates same-model and all-model at both the 2td and 5td horizons, factor rates for drop_type/cause/intraday_state, sample counts). Read its JSON output. Exit 4 = some due rows couldn't fetch history — retry those tickers manually per G1 (fallback source: finance.yahoo.com/quote/TICKER/history), update the rows yourself, then run grade.py once more (it never re-grades; the extra run refreshes calibration.json to include the hand-graded rows). Sanity-check 1-2 graded values against the raw source before trusting a surprising grade (per G3).
...
S8 LOG — append this run's picks to predictions.csv, one row per (run_date,run_time,ticker): skip any ticker already logged for this same run_date + run_time. The same ticker MAY be logged again on a later run the same day (different run_time). New row: run_date=today, run_time=this run's time, model=this run's model id, ticker, rebound_pct, ref_price=current price, status=pending, graded fields blank, plus all feature columns (drop_pct, sector, drop_type, cause, intraday_state, vol_signature, earnings_soon, corr_group) from S3. Append this run's row to context.csv. Commit predictions.csv, context.csv, calibration.json (plus ROUTINE.md if updated, scripts/*.py when fixed under the SCRIPT-FIRST RULE, and logic_changes.csv when a row was appended) to main — or to the harness session branch when main pushes are restricted; CI fast-forwards main ("log <date> <run_time> picks"). No PR.
CLOSED-TAPE RE-RUN rule: if the market is closed at run time AND an earlier closed-slot run already logged this run_date on an identical tape (same capped set at the same ref prices), skip the S7 push and skip appending S8 pick rows — but STILL append this run's context.csv row and commit it. Grading (S0) always runs regardless.
